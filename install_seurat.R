...
install_bioc_pkg <- function(pkg_name) {
  cat(sprintf("  %s (BiocManager)... ", pkg_name))
  ...
  tryCatch({
    BiocManager::install(pkg_name, ask = FALSE, update = FALSE, quiet = TRUE)
    if (pkg_name %in% installed.packages()[, "Package"]) {
      cat("success\n")
      return(TRUE)
    }
...
install_github_pkg_simple <- function(repo, pkg_name) {
  cat(sprintf("  %s (GitHub)... ", pkg_name))
...
cat("  Installing hexbin dependencies via BiocManager...\n")
install_bioc_pkg("marray")
install_bioc_pkg("affy")
install_bioc_pkg("Biobase")
install_bioc_pkg("limma")
...
cat("  After downloading, run: install.packages('path/to/BPCells-main', type='source', repos=NULL)\n")

cat("  Installing presto and glmGamPoi via GitHub...\n")
install_github_pkg_simple("presto", "presto")
install_github_pkg_simple("Bioconductor/glmGamPoi", "glmGamPoi")
